...
public class RedisUtil {
    ...
    private String buildKeyPrefix(String key) {
        ...
        if (!key.startsWith(RedisConstant.KEY_PREFIX)) {
            return RedisConstant.KEY_PREFIX.concat(key);
        }
        return key;
    }
    ...
    public void hmSet(String key, Object hashKey, Object value) {
        try {
            key = buildKeyPrefix(key);
            HashOperations<String, Object, Object> hash = redisTemplate.opsForHash();
            hash.put(key, hashKey, value);
            redisTemplate.expire(key, RedisConstant.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        } catch (Exception e) {
